"""
MF-AI-Zero - v0.8b candidate-osszehasonlito.

Lefuttatja a test_v07e_router.py teljes tesztkeszletet TOBBSZOR (alapbol
5x) mindegyik megadott altalanos modellel, es osszehasonlitja:
...
Futtatás:
    python tests/compare_v08b_candidates.py
"""
...
MODELS = {
    ...
    "v0.8b_candidate_b": os.path.join(config.BASE_DIR, "models", "mf_ai_zero_chat_v0_8b_b.pt"),
}
...
def main():
    ...
    header = f"{'Modell':<28}{'ált.átlag':<12}{'ált.min':<10}{'mondatsz.átlag':<16}{'routerhiba':<12}{'megfelel?'}"
    print(header)
    print("-" * len(header))
    ...
    for label, s in summary.items():
        ...
        meets_all = meets_avg and meets_min and meets_sentence and meets_router

        print(f"{label:<28}{s['general_avg']:<12.1f}{s['general_min']:<10}"
              f"{s['sentence_avg']:<16.1f}{s['wrong_model_total']:<12}"
    ...
    print(f"\nElfogadási küszöbök: átlag >= {MIN_GENERAL_AVERAGE}/20 ÉS minden futás >= "
          f"{MIN_GENERAL_SINGLE_RUN}/20 ÉS mondatszám átlag >= 19/20 ÉS routerhiba == 0")
